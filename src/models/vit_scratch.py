"""
Vision Transformer 从零实现版本（vit_scratch.py）
功能：完全手动实现 ViT 的所有组件（Embedding、Self-Attention、Encoder 等）
对应原项目 model.py 中的 VisionTransformer 类
"""
...
class EmbedLayer(nn.Module):
    ...
    def __init__(self, n_channels, embed_dim, image_size, patch_size, dropout=0.0):
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(1, (image_size // patch_size) ** 2, embed_dim),
                                          requires_grad=True)  # Learnable Positional Embedding
    ...
    def forward(self, x):
        B = x.shape[0]
        x = self.conv1(
            x)  # B, C, IH, IW     --> B, E, IH/P, IW/P                Split image into the patches and embed patches
        x = x.reshape(
            [B, x.shape[1], -1])  # B, E, IH/P, IW/P --> B, E, (IH/P*IW/P) --> B, E, N    Flattening the patches
        x = x.permute(0, 2,
                      1)  # B, E, N          --> B, N, E                         Rearrange to put sequence dimension in the middle
        x = x + self.pos_embedding  # B, N, E          --> B, N, E                         Add positional embedding
        x = torch.cat((torch.repeat_interleave(self.cls_token, B, 0), x),
                      dim=1)  # B, N, E          --> B, (N+1), E       --> B, S, E    Add classification token at the start of every sequence
        x = self.dropout(x)
        return x
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        xq = self.queries(x).reshape(b, s, self.n_attention_heads,
                                     self.head_embed_dim)  # B, Q, E      -->  B, Q, (H*HE)  -->  B, Q, H, HE
        xq = xq.permute(0, 2, 1, 3)  # B, Q, H, HE  -->  B, H, Q, HE
        xk = self.keys(x).reshape(b, s, self.n_attention_heads,
                                  self.head_embed_dim)  # B, K, E      -->  B, K, (H*HE)  -->  B, K, H, HE
        xk = xk.permute(0, 2, 1, 3)  # B, K, H, HE  -->  B, H, K, HE
        xv = self.values(x).reshape(b, s, self.n_attention_heads,
                                    self.head_embed_dim)  # B, V, E      -->  B, V, (H*HE)  -->  B, V, H, HE
        xv = xv.permute(0, 2, 1, 3)  # B, V, H, HE  -->  B, H, V, HE

        # Compute Attention presoftmax values
        xk = xk.permute(0, 1, 3, 2)  # B, H, K, HE  -->  B, H, HE, K
        x_attention = torch.matmul(xq,
                                   xk)  # B, H, Q, HE  *   B, H, HE, K   -->  B, H, Q, K    (Matmul tutorial eg: A, B, C, D  *  A, B, E, F  -->  A, B, C, F   if D==E)
        ...
        x = torch.matmul(x_attention,
                         xv)  # B, H, Q, K  *  B, H, V, HE  -->  B, H, Q, HE     Compute Attention product with Values
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, n_channels, embed_dim, n_layers, n_attention_heads, forward_mul, image_size, patch_size,
        ...
        self.encoder = nn.ModuleList(
            [Encoder(embed_dim, n_attention_heads, forward_mul, dropout=dropout) for _ in range(n_layers)])
